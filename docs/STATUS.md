...
12. **Mattermost activity and membership revisions** -- only complete authoritative snapshot replacement changes existing team/channel `is_active` state; ordinary conflicting upserts update entity data without reactivating retired rows, while genuinely new rows insert active. Membership cache ordering uses the server's `last_update_at`; `last_viewed_at` remains the independent read position, and an omitted revision deterministically merges at revision zero.
