...
func TestMattermostSnapshotAdaptersPreserveRawIdentityAndRebuildNames(t *testing.T) {
	...
	snapshot := service.ServerSnapshot{
		...
		Sections:     []service.ChannelSection{{ID: "server:s1:direct", Kind: service.ChannelSectionKindDirect, Channels: []service.ChannelEntry{{Channel: mattermost.Channel{ID: "dm", ServerID: "s1", Name: "me__peer", DisplayName: "Direct display", Kind: mattermost.ChannelKindDirect, UpdatedAt: 13}, Membership: &mattermost.ChannelMembership{ChannelID: "dm", UserID: "me", LastViewedAt: 77, MsgCount: 3}, DisplayName: "Alice"}}}},
		ChannelUsers: map[string][]string{"dm": {"me", "peer"}},
	}
	raw := mattermostCacheSnapshot(snapshot, observedAt)
	if raw.Server.LastSyncedAt != 1234 || raw.Channels[0].DisplayName != "Direct display" || raw.Memberships[0].UpdatedAt != 77 || !reflect.DeepEqual(raw.ChannelUsers["dm"], []string{"me", "peer"}) {
